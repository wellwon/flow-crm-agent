      ...
      {isOpen && (
        ...
        >
          ...
          <motion.div
            ...
            className="relative matte-glass w-full max-w-3xl max-h-[85vh] overflow-hidden flex flex-col"
          >
            {/* Header */}
            <div className="p-5 border-b border-border flex items-center justify-between">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-[10px] font-mono text-primary uppercase tracking-wider">JARVIS · Утренний брифинг</span>
                </div>
                <h2 className="text-lg font-semibold text-foreground mt-1 capitalize">{today}</h2>
              </div>
              <button onClick={onClose} className="p-2 rounded-[8px] hover:bg-primary/10 hover:text-primary transition-colors" aria-label="Close">
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-5">
              ...
              <BriefingSection title="ЗАДАЧИ НА ДЕНЬ" icon={CheckCircle} iconColor="text-node-completed" badge="6 штук · ~4.5 часа">
                <div className="space-y-2">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 p-2.5 rounded-[10px] bg-card border border-border hover:border-primary/30 transition-colors cursor-pointer group">
                      <span className="text-[11px] text-foreground flex-1">{item.task}</span>
                      {item.draft && <span className="text-[9px] font-mono px-1.5 py-0.5 rounded-[6px] bg-primary/10 text-primary">Черновик</span>}
              ...
              <BriefingSection title="НОЧНОЙ МОНИТОРИНГ" icon={Search} iconColor="text-primary">
                <div className="space-y-2">
                  ...
                  {[
                    ...
                    <div key={i} className="flex items-center gap-3 p-2 rounded-[10px] bg-card border border-border">
                      <span className="text-[11px] text-foreground">{t.eq} — {t.city}</span>
                      <span className="text-[10px] font-mono text-primary ml-auto">{t.amount}</span>
              ...
              <BriefingSection title="КОМАНДА" icon={Users} iconColor="text-primary">
                ...
                <div className="mt-3 p-2.5 rounded-[10px] bg-node-active/10 border border-node-active/20">
            ...
            <div className="p-4 border-t border-border flex items-center gap-3">
              <button className="flex-1 py-2.5 rounded-[10px] bg-primary text-primary-foreground text-xs font-semibold hover:brightness-110 transition-all">
                Принять план дня
              </button>
              <button className="px-4 py-2.5 rounded-[10px] border border-border text-muted-foreground text-xs font-medium hover:text-primary hover:border-primary/30 hover:bg-primary/10 transition-all">
                Изменить
              </button>
            </div>
          </motion.div>
...
function BriefingCard({ title, items, actions }: { title: string; items: string[]; actions: string[] }) {
  return (
    <div className="p-3 rounded-[10px] bg-card border border-border mb-2">
      ...
      <div className="flex gap-2 mt-2">
        {actions.map((a, i) => (
          <button key={i} className="text-[10px] px-2.5 py-1 rounded-[8px] bg-primary/15 text-primary hover:bg-primary/25 transition-colors">{a}</button>
        ))}
      </div>
    </div>
  );
...
function TeamRow({ name, status, detail }: { name: string; status: 'ok' | 'warning' | 'blocked'; detail: string }) {
  return (
    <div className="flex items-center gap-3 p-2 rounded-[10px] bg-card border border-border">
