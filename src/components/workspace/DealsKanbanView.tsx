          ...
          >
            ...
            <div className="matte-glass px-3 py-2 mb-2 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] font-medium text-foreground">{dealStatusLabels[status]}</span>
            ...
            {/* Drop zone */}
            <div
              className={`flex-1 flex flex-col gap-2 rounded-[14px] p-1.5 transition-all ${
                isOver && !isSameColumn
            ...
            >
              {colDeals.map((deal) => (
                <div
                  ...
                  className={`matte-glass p-3 cursor-grab active:cursor-grabbing
                    hover:border-primary/30 transition-all group
                    ${draggedDealId === deal.id ? 'opacity-50 scale-95' : ''}`}
                >
                  <div className="flex items-start justify-between mb-1">
                    <div className="font-medium text-[13px] text-foreground group-hover:text-primary transition-colors flex-1 mr-2">
                      {deal.title}
                    </div>
                    <GripVertical className="w-3.5 h-3.5 text-muted-foreground/40 group-hover:text-muted-foreground flex-shrink-0 mt-0.5" />
                  </div>
                  <div className="text-[12px] text-muted-foreground mb-3">{deal.company}</div>
                  ...
                  <div className="flex items-center justify-between mt-2">
                    ...
                    <span className="text-[11px] text-muted-foreground">{deal.progress}%</span>
                  </div>
                </div>
              ))}

              {colDeals.length === 0 && (
                <div className={`flex-1 flex items-center justify-center rounded-[14px] border border-dashed transition-colors min-h-[100px] ${
